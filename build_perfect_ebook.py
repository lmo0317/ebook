...
TYPO_REPLACEMENTS = [
    ...
    (r"\b시권스\b", "시퀀스"),
    (r"파인[튜특류뉴듯][님낭빌딩넣]|파인튜-(?=\s*\(Fine-tuning\))", "파인튜닝"),
    (r"파인튜닝이[관과]", "파인튜닝이란"),
    (r"파인듯낭", "파인튜닝"),
    (r"파인듯넣", "파인튜닝"),
    (r"보젯습니다", "보겠습니다"),
    ...
    (r"(?<!플)\b([가-힣]+)랫(습|어|던|다|고)\b", r"\1했\2"),
...
def apply_typo_fixes(text: str) -> str:
    # 1. Apply ground-truth learned corrections knowledge base first
    t = apply_learned_knowledge(text)
